      ...
      <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        >
          <div className="flex flex-col items-center text-center">
            <Avatar size={72} style={{ background: adminPalette.red, color: '#fff', fontSize: 26, fontWeight: 600 }}>
              {initials}
            </Avatar>
            <div className="mt-4 text-[18px] font-semibold" style={{ color: adminPalette.ink }}>
              {user?.fullName}
            </div>
            <div className="mt-1 text-sm" style={{ color: adminPalette.textMuted }}>
              System administrator
            </div>
            <div
              className="mt-4 w-full rounded-xl px-4 py-3 text-left text-sm"
              style={{ background: adminPalette.panelAlt, color: adminPalette.inkSoft }}
            >
              <div className="flex items-center gap-2">
                <MailOutlined style={{ color: adminPalette.textMuted }} />
                <span className="truncate">{user?.email || 'No email on file'}</span>
              </div>
              <div className="mt-2 flex items-center gap-2">
                <PhoneOutlined style={{ color: adminPalette.textMuted }} />
            ...
            >
              <div className="grid gap-x-4 sm:grid-cols-2">
                <Form.Item name="fullName" label="Full name" rules={[{ required: true, message: 'Full name is required' }]}>
                  <Input prefix={<UserOutlined style={{ color: adminPalette.textMuted }} />} />
                </Form.Item>
                <Form.Item name="email" label="Email" rules={[{ type: 'email', message: 'A valid email is required' }]}>
                  <Input prefix={<MailOutlined style={{ color: adminPalette.textMuted }} />} />
                </Form.Item>
              </div>
              <Form.Item name="phone" label="Phone">
                <Input prefix={<PhoneOutlined style={{ color: adminPalette.textMuted }} />} style={{ maxWidth: 280 }} />
              </Form.Item>
            ...
            <Form form={passwordForm} layout="vertical" onFinish={handlePasswordSubmit} onValuesChange={() => setPasswordSaved(false)}>
              <div className="grid gap-x-4 sm:grid-cols-3">
                <Form.Item name="currentPassword" label="Current password" rules={[{ required: true, message: 'Current password is required' }]}>
                  <Input.Password prefix={<LockOutlined style={{ color: adminPalette.textMuted }} />} />
                </Form.Item>
